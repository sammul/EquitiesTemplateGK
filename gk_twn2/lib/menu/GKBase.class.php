...
if (!defined('_GK_BASE_MENU_CLASS')) {
    ...
    class GKMenuBase extends JObject {
        ...
        function loadMenu($menuname = 'mainmenu') {
            ...
            foreach ($rows as $index => $v) {
                ...
                if ($v->access <= $aid) {
                    ...
                    $iParams = $this->createParameterObject($v->gkparams);
        ...
        function genMenuItem($item, $level = 0, $pos = '', $ret = 0)
        {
            $data = '';
            $tmp = $item;
            $tmpname = ($this->getParam('gkmenu') && !$tmp->gkparams->get('showtitle', 1)) ?
                '' : $tmp->name;
           ...
           $active = $this->genClass($tmp, $level, $pos);
            ...
            $id = 'id="menu' . $tmp->id . '"';
            $iParams = $item->gkparams;
            $tmpname = str_replace('"','&quot;', $tmpname);
            $txt = '<span class="menu-title">' . $tmpname . '</span>';

            if ($tmp->gkparams->get('desc')) {
                $txt .= '<span class="menu-desc">' . JText::_($tmp->gkparams->get('desc')) . '</span>';
            }
			
			if($tmp->gkparams->get('menu-anchor_title', '') == '') {
            $title = "title=\"$tmpname\"";
			} else {
			 $title = "title=\"".$tmp->gkparams->get('menu-anchor_title', '')."\"";	
			}
