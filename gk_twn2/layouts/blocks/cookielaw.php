<!-- Begin Cookie Consent plugin by Silktide - http://silktide.com/cookieconsent -->
     <?php if($this->getParam('cookie_consent', '0') == '1') : ?>
          <!-- cookie conset latest version or not -->
          <?php if($this->getParam('cookie_latest_version', '0') == '0') : ?>
               <?php if($this->getParam('cookiecss', '1') == '1') : ?><link rel="stylesheet" type="text/css" href="https://s3-eu-west-1.amazonaws.com/assets.cookieconsent.silktide.com/1.0.7/style.min.css"/><?php endif; ?>
          ...
          <?php else : ?>
               <?php if($this->getParam('cookiecss', '1') == '1') : ?><link rel="stylesheet" type="text/css" href="https://s3-eu-west-1.amazonaws.com/assets.cookieconsent.silktide.com/current/style.min.css"/><?php endif; ?>
          ...
          cc.initialise({
               ...
               settings: {
                    bannerPosition: <?php echo '"'.$this->getParam('banner_position', 'bottom').'"'; ?>,
                    consenttype: <?php echo '"'.$this->getParam('consenttype', 'explicit').'"'; ?>,
                    onlyshowbanneronce: false,
                    style: <?php echo '"'.$this->getParam('cookie_style', 'light').'"'; ?>,
                    refreshOnConsent: <?php if($this->getParam('refreshOnConsent', '0') == '1') : ?>true<?php else : ?>false<?php endif; ?>,
                    useSSL: <?php if($this->getParam('cookie_use_ssl', '0') == '1') : ?>true<?php else : ?>false<?php endif; ?>,
                    tagPosition: <?php echo '"'.$this->getParam('banner_tag_placement', 'bottom-right').'"'; ?>
               },
               strings: {
