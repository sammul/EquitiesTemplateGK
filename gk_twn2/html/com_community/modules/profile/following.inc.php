...
$friends = CFactory::getModel('following'); // <--- focus on this
$sorted = $jinput->get->get('sort', 'latest', 'STRING'); //JRequest::getVar( 'sort' , 'latest' , 'GET' );
$filter = JRequest::getWord('filter', 'following', 'GET');
...
if ($config->get('alphabetfiltering')) {
	$filterItems = array(
		...
		'following' => JText::_('COM_COMMUNITY_JUMP_ALL'),
...
$html = $tmpl->set('isMine', $isMine)
		...
		->set('friendRel', "following")
		->fetch('friends.list');

$html .= '<div class="cPagination">';
$pagination = $friends->getPagination();
$html .= $pagination->getPagesLinks("#following");
$html .= '</div>';
echo $html;