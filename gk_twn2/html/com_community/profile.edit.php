...
$user_parsing = & JFactory::getUser($my->id);
?>
...
<div class="cLayout cProfile-Edit">
	<form name="jsform-profile-edit" id="frmSaveProfile" action="<?php echo CRoute::getURI(); ?>" method="POST" class="cForm community-form-validate">
		...
		<ul class="cTabsMenu cResetList cFloatedList clearfix">
			<li><a href="#basicSet" id="basicSetLink"><?php echo JText::_('COM_COMMUNITY_PROFILE_SETTING_INFO'); ?></a></li>
			<li><a href="#avatarSet" id="avatarSetLink"><?php echo JText::_('COM_COMMUNITY_PROFILE_AVATAR_EDIT'); ?></a></li>
			<li><a href="#detailSet" id="detailSetLink"><?php echo JText::_('COM_COMMUNITY_PROFILE_SETTING_INFO_DETAILS'); ?></a></li>
			<?php if (in_array(9, $user_parsing->groups)): ?><li><a href="#irdetailSet" id="irdetailSetLink"><?php echo JText::_('COM_COMMUNITY_PROFILE_IRDETAILS'); ?></a></li><?php endif; ?>
			<!--<li><a href="#socialSet" id="socialSetLink"><?php echo JText::_('Social Settings'); ?></a></li>-->
			<li><a href="/component/broadcast/?view=config" data-proceed="yes"><?php echo JText::_('Social Settings'); ?></a></li>
		</ul>

		<div class="cTabsContent">
			<div id="basicSet" class="section"> <!-- Profile Basic Setting -->
				<?php
				$parsing_user = & JFactory::getUser();
				$ir = false;
				$ir_data = '';
				foreach ($fields as $name => $fieldGroup) {
					if ($ir) {
						$ir = false;
						$ir_data = ob_get_contents();
						ob_end_clean();
					}

					//Investing Information hide from issuers and authors
					if (strcasecmp($name, "Investing Information") == 0) {
						if (in_array(9, $parsing_user->groups) || in_array(3, $parsing_user->groups)) {
							continue;
						}
					}
					//Issuer information only for issuers
					if (strcasecmp($name, "Issuer Information") == 0) {
					...
					//Social Networks information only for authors
					if (strcasecmp($name, "Social Networks") == 0 || strcasecmp($name, "Writer Information") == 0) {
...
//							echo "</pre>";
			...
			<div id="irdetailSet"><?php echo $ir_data; ?></div>
			...
			<div id="detailSet" class="section"> <!-- Profile Detail Setting -->
				...
				<ul class="cFormList cFormHorizontal cResetList">
					<!-- username -->
				    <?php if (in_array(9, $parsing_user->groups)) { ?>
					<li>
				   ...
				   <?php } else { ?>
					<li>
						<label class="form-label" for="username"><?php echo JText::_('COM_COMMUNITY_PROFILE_USERNAME'); ?></label>
						<div class="form-field">
                            <input class="input text" type="text" id="username" name="username" size="40" value="<?php echo $this->escape($user->get('username')); ?>" />
						</div>
					</li>
				    <?php } ?>
					...
					<!-- DST -->
					<li class="has-seperator" style="display:none''">
			...
			<ul class="cFormList cFormHorizontal cResetList" id="saveJomDataSection">
	...
	<div id="socialSet"><?php require_once 'modules/profile/broadcast_settings.inc.php'; ?></div>
	<div id="avatarSet"><?php require_once 'modules/profile/uploadavatar.inc.php'; ?></div>
...
<script type="text/javascript">
	...
	joms.jQuery( document ).ready( function(){
		...
		joms.jQuery('.cTabsMenu li a').click(function () {
			...
			if(joms.jQuery(this).data("proceed"))
				return true;
			return false;
		}).filter(filter).click();
	...
	jQuery(document).ready(function(){
		jQuery("#socialSetLink").click(function(){jQuery("#saveJomDataSection").hide();});
		jQuery("#avatarSetLink").click(function(){jQuery("#saveJomDataSection").hide();});
		jQuery("#basicSetLink").click(function(){jQuery("#saveJomDataSection").show();});
		jQuery("#detailSetLink").click(function(){jQuery("#saveJomDataSection").show();});
		jQuery("#irdetailSetLink").click(function(){jQuery("#saveJomDataSection").show();});
		
		var hash=window.location.hash;
		switch(hash){
			case '#avatarSet':
				jQuery("#avatarSetLink").click()
				break;
		}
	});
</script>
