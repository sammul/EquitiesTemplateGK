					...
					<div class="cIndex-Box clearfix">
						<div style="float: left; width: 160px">
							<a href="<?php echo $row->profileLink; ?>" class="cIndex-Avatar cFloat-L">
								<img class="cAvatar-bigone" src="<?php echo $row->user->getThumbAvatar(); ?>" alt="<?php echo $row->user->getDisplayName(); ?>" />
								<?php if ($row->user->isOnline()): ?>
									<div class="cStatus-Online">
										<?php echo JText::_('COM_COMMUNITY_ONLINE'); ?>
									</div>
								<?php endif; ?>
							</a>
						</div>
						<div class="cIndex-Content-bigone">
							<h3 class="cIndex-Name cResetH">
								<a href="<?php echo $row->profileLink . "#quote"; ?>"><?php echo $row->user->getDisplayName(); ?>&nbsp;(<?php echo $row->user->username; ?>)</a>
							</h3>
							<?php
							$userStatus_102813 = $row->user->getStatus();
							if(trim($userStatus_102813)!=''){ ?>
								<div class="cIndex-Status"><?php echo $row->user->getStatus(); ?></div>
							<?php } ?>
							
								<?php
								$user_102813 = &JFactory::getUser();
								//print_r($user_102813->groups);
								if ((isset($user_102813->groups) && !in_array(9, $user_102813->groups)) || empty($user_102813->groups)) {
									?>
							<div class="cIndex-Actions">
									...
									<?php
										...
										<?php
										} else if($my->id == $row->user->id) {
                                            //do nothing;
										} else {
                                            $isFollwingAlready = CExtraHelper::followingTo($my->id, $row->user->id);
                                            if ($isFollwingAlready) {
                                        ?>
											<div>
								...
								<?php
									...
									?>
									<style>.cIndex-Content-bigone{min-height: 0 !important}</style>
									<?php
								}
								?>
						</div>
						<div class="cIndex-Content-bigone" style="margin-top:10px">
							<?php echo $row->user->getInfo("FIELD_ABOUTME"); ?>
						</div>
					</div>
				</li>
			<?php endif; ?>
