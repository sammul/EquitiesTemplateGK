...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2UsersBlock<?php if($params->get('moduleclass_sfx')) echo ' '.$params->get('moduleclass_sfx'); ?>">
	...
	<ul>
		...
		<li class="<?php echo ($key%2) ? "odd" : "even"; if(count($users)==$key+1) echo ' lastItem'; ?>">
			...
			$module_itemid=$params->get('module_itemid');
		//	$newlink=JURI::base().'index.php?option=com_community&view=profile&userid='.$newuserid.'&Itemid='.$module_itemid;
			$newlink=CRoute::_('index.php?option=com_community&view=profile&userid='.$newuserid);
			
			 if($userAvatar && !empty($user->avatar)): ?>

			<a class="k2Avatar ubUserAvatar" rel="author" href="<?php echo $newlink;//$user->link; ?>" title="<?php echo K2HelperUtilities::cleanHtml($user->name); ?>">
			...
			<a class="ubUserName" rel="author" href="<?php echo $newlink;//$user->link; ?>" title="<?php echo K2HelperUtilities::cleanHtml($user->name); ?>">
