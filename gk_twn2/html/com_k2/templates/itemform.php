...
$itemformtype = JRequest::getVar("itemformtype", "0");

$message = "";
switch ($itemformtype) {
	case '109': case '110':
		$message = "Thank you for submitting a blog post.";
		break;
	default:
		$message = "Thank you. Your article has been submitted for review.";
}
...
$document->addScriptDeclaration("
	Joomla.submitbutton = function(pressbutton){
		...
		if (\$K2.trim(\$K2('#title').val()) == '') {
		...
		else if (\$K2.trim(\$K2('#K2ExtraField_9').val()) == '' && '".JRequest::getVar("itemformtype", 0)."'=='1'){
			alert( '".JText::_('K2_DISCLOSURE_CANT_BE_EMPTY')."' );
		}
		else {
            if (pressbutton == 'save') {
                alert( '" . $message . "' );
			...
			if(validation === true) {
                ...
                if (pressbutton == 'saveDraft') {
                    \$K2('#adminForm').attr('target', 'previewwindow');
                    window.open('about:blank', 'previewwindow', 'height=600, width=1000, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no');

                    submitform( pressbutton );
                    \$K2('#adminForm').attr('target','_parent');
                } else {
				    submitform( pressbutton );
                }
			}
			...
			<div id="k2FrontendContainer">
				<div id="k2Frontend">
					<table class="k2FrontendToolbar" cellpadding="2" cellspacing="4">
						<tr>
							<?php if (JRequest::getVar("itemformtype") == 1) { ?>
					...
					<div id="k2FrontendEditToolbar">
						<h2 class="header icon-48-k2">
							<?php
							switch (JRequest::getVar("itemformtype", "0")) {
								case '1':
									$k2item_sufix = "Article";
									break;
								case '66':
									$k2item_sufix = "Press Release Entry";
									break;
								case '67':
									$k2item_sufix = "Blog Entry";
									break;
								case '109': case '110':
									$k2item_sufix = "Blog";
									break;
							...
							?>
							<?php echo (JRequest::getInt('cid')) ? JText::_("Equities.Com Edit $k2item_sufix") : JText::_("Equities.Com Add $k2item_sufix"); ?>
						</h2>
					</div>
					<div class="clr"></div>
					<hr class="sep" />
                        ...
                        <tr>
                            <td>
                                <table class="adminFormK2">
                                    ...
                                    <tr>
                                        <td class="adminK2LeftCol">
                                            <label><?php echo JText::_('Tickers'); ?></label>
                                        </td>
                                        <td class="adminK2RightCol">
                                ...
                                <div class="simpleTabs" id="k2Tabs">
                                    <ul class="simpleTabsNavigation">
										...
										<?php if ($this->params->get('showImageTab') && $itemformtype != 67): ?>
					...
					<!--<li id="tabVideo"><a href="#k2Tab4"><?php // echo JText::_('K2_MEDIA');       ?></a></li>-->
                                    ...
                                    <div class="simpleTabsContent" id="k2Tab1">
										<?php if ($this->params->get('mergeEditors')): ?>
											...
											<?php if (JRequest::getVar("itemformtype", 0) == 1) { ?>

												<!-- Tab extra fields -->
												<?php
												$disclosure_label = 'Disclosure: Please state any and all relevant disclosures pertaining to the article you are submitting. If none, put "None".';
												$disclosure_value = '';
												$notes_label = "Notes to Editor";
												$notes_value = "";

												if (count($this->extraFields)) {
													foreach ($this->extraFields as $extraField) {
														if ($extraField->id == 9) {
															$disclosure_label = $extraField->name;
															$disclosure_value = strip_tags($extraField->element);
														}
														if ($extraField->id == 10) {
															$notes_label = $extraField->name;
															$notes_value = strip_tags($extraField->element);
														}
													}
												}
												?>
												<label for="K2ExtraField_9"><?php echo $disclosure_label; ?></label>
												<textarea name="K2ExtraField_9" id="K2ExtraField_9" style="width:100%; height:100px"><?php echo $disclosure_value; ?></textarea><br /><br />
												<label for="K2ExtraField_10"><?php echo $notes_label; ?></label>
												<textarea name="K2ExtraField_10" id="K2ExtraField_10" style="width:100%; height:100px"><?php echo $notes_value; ?></textarea>
												<div class="clr"></div>
											<?php } ?>
											<div>
												<table class="k2FrontendToolbar" cellpadding="2" cellspacing="4">
...
<script>
	jQuery(document).ready(function(){
		var itemformtype="<?php echo JRequest::getVar("itemformtype", "0"); ?>";
		if(itemformtype != "0"){
			jQuery("#catid").val(itemformtype);
		}
	});
</script>